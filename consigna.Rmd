...
además de registar el sexo de animal, pesaron la carcasa en kg. 
y midieron la grasa como la profundida de la misma a la altura de la doceava
costilla
...
datos. Luego, la función `aes` indica que use la columna `Kind` para el eje
x y `Fatness` para el y. Luego agregamos que queremos hacer puntos con esos 
datos con `geom_point`.

Aunque así es medio díficil ver si hay diferencias, podemos agregar un boxplot
por encima
...
gráfican en el orden que se llaman. De otra forma la caja taparía a los puntos.
...
Fatness by Kind (grasa por tipo (sexo)). Y nos da el valor del estadístico t
seguido por los grados de libertad (`gl`) y la probabildad de un valor `t`
...
Como tenemos dos variables independientes, una cualitiva y otra continua, usamos
...
Analisemos lo que hemos hecho arriba. Primero, llamamos a la función `lm` que
...
factores alfabeticamente. Por lo tanto, `(Intercept)` corresponde a la ordenada
al origen de los carneros. ¿Y que representa `ClaseOveja`? No es la ordenda al
...
jugo a nuestros datos. La varianza del estimador aumenta a medida que se aleja 
...
tiempo y luego fueron faenados. Para descargar los datos utilizen este comando
